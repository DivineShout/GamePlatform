<?php include ROOT . '/views/layouts/header.php'; ?>
<section class="greeting">
    <section class="welcome">
    <?php if ($result): ?>
       <center> <h3>Дані відредаговані!</h3></center>
        <center><a href="/cabinet" class="butnlider">Профіль</a></center>
    <?php else: ?>
    <?php if (isset($errors) && is_array($errors)): ?>
        <ul>
            <?php foreach ($errors as $error): ?>
                <li> - <?php echo $error; ?></li>
            <?php endforeach; ?>
        </ul>
    <?php endif; ?>

        <center><h1 >Заповніть поля!</h1></center>
        <div class="content-position">
            <div class="form-look">
                <form class="form" action="#" method="post" enctype="multipart/form-data">

                    <h3>Ім'я:</h3>
                    <input class="register" type="text" placeholder="Ім'я" name="name" value="<?php echo $name; ?>" />
                    <h3>Прізвище:</h3>
                    <input class="register" type="text" placeholder="Фамілія" name="surname" value="<?php echo $surname; ?>" />
                    <h3>Дата народження:</h3>
                    <input class="register" type="date" placeholder="Дата народження" name="data_birth" value="<?php echo $data_birth; ?>"  />
                    <h3>Нік:</h3>
                    <input id="data_r" class="register" type="text" placeholder="Нік" name="nick"  value="<?php echo $nick; ?>" />

                    <h3>Пароль:</h3>
                    <input class="register" type="password" placeholder="Пароль" name="password" value="<?php echo $password; ?>" />
                    <h3>Зображення:</h3>
                    <center><label id="gtrg" class="register"><input class="register" type="file" name="image"/>Завантажити нове зображення</label></center>
                    <br>

                    <center> <input type="submit" name="submit" class="butnlider" value="Зберегти" /></center>
                </form>
            </div>
        </div>


        <?php endif; ?>
        <br><br><br><br><br><br><br><br>
    </section>

</section>

<?php include ROOT . '/views/layouts/footer.php'; ?>
